...
# a = '优酷携手小猪佩奇母公司eone 孵化全新全球ip BIOOBIIIOOOOOOOOOOOOOOOOO'
# b = ['后来', '坦言当时自己']
...
with open('data/coreEntityEmotion_train.txt', 'r') as f:
    for line in f:
        ...
        source_list.append(title.strip())
        target_list.append(source_to_target(title.strip(), entities))
                ...
                if len(content) > 160:
                    ...
                    for c in c_list:
                        if (len(c) > 8 and len(c) < 160) or has_entity(c, entities):
                            if len(c) > 160:
                                c = c[:160]
                            source_list.append(c.strip())
                            target_list.append(source_to_target(c.strip(), entities))
                            assert len(source_list[-1]) == len(target_list[-1])
                    ...
                    source_list.append(content.strip())
                    target_list.append(source_to_target(content.strip(), entities))
                    assert len(source_list[-1]) == len(target_list[-1])
...
valid_data = c[train_len:valid_len]
test_data = c[valid_len:]
save_data('data/train.txt', train_data)
save_data('data/valid.txt', valid_data)
save_data('data/test.txt', test_data)

with open('data/source.txt', 'w') as w1:
    w1.write('\n'.join(source_list))

with open('data/target.txt', 'w') as w2:
    w2.write('\n'.join(target_list))
